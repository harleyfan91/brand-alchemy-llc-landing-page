...
const outFile = join(outDir, 'services-3-photos.pdf')
...
const defaultCollagePath = join(repoRoot, 'public', 'pdf-assets', 'services-photos-collage-sample.png')
/** Native pixels of `services-photos-collage-sample.png` — update if you replace the asset. */
...
const S = StyleSheet.create({
  ...
  /** Slightly smaller than gym — longer document title (“Professional Services”). */
  titleHeadline: {
    fontSize: 12.85,
 ...
 */
const content = {
  title: '3 Photos You Must Upload This Month: Professional Services',
  subtitle: 'A visual blueprint for Google Maps & Yelp Presence',
  collageImageSrc: defaultCollagePath,
  angles: [
    {
      title: 'Straight on “Trust” Interior',
      frame:
        'Eye-level shot of a clean, professional space; a doctor’s reception or exam room, attorney’s desk or conference table, a mechanic’s tidy work bay.',
      light: 'Bright, even lighting; ensure the environment feels clean and professional.',
    },
    {
      title: 'Three Quarter “Work in Context” Shot',
      frame:
        'Show professionals actively at work. Attorneys in discussion with a client, doctors consulting with a patient or a mechanic working under the hood.',
      light:
        'Use natural or well-balanced interior lighting; highlight the professional’s focus.',
    },
    {
      title: 'Detail / Close Up “Proof”',
      frame:
        'Show tangible details. Close-up of a document signing (redact sensitive text), a neat arrangement of sanitized instruments or before/after of a replaced part.',
      light: 'Bright, even lighting with sharp focus.',
      tip: 'Ensure details are crisp; avoid clutter.',
    },
  ],
}
...
registerBrandPdfFonts()
const buf = await renderToBuffer(h(ServicesPhotosDoc))
