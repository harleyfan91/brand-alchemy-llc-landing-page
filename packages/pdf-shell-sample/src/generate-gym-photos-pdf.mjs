...
const outFile = join(outDir, 'gym-3-photos.pdf')
...
const defaultCollagePath = join(repoRoot, 'public', 'pdf-assets', 'gym-photos-collage-sample.png')
/** Native pixels of `gym-photos-collage-sample.png` — update if you replace the asset. */
...
const S = StyleSheet.create({
  ...
  /** Small enough to stay on one line in the gutter width and keep the page to a single sheet. */
  titleHeadline: {
    fontSize: 14.25,
 ...
 */
const content = {
  title: '3 Photos You Must Upload This Month: Gym/Fitness Studio',
  subtitle: 'A visual blueprint for Google Maps & Yelp Presence',
  collageImageSrc: defaultCollagePath,
  angles: [
    {
      title: 'Wide Establishing Shot of Facility',
      frame:
        'Capture from a front corner to show layout, equipment, mirrors, and open space.',
      light:
        'Use indoor lights and natural light from windows if available; avoid dark or shadowy areas.',
    },
    {
      title: 'Action/Class Shot',
      frame: '45° side angle of a group class or busy section; freeze motion for energy.',
      light: 'Ensure clear exposure so participants and form are visible.',
    },
    {
      title: 'Trainer w/Client Shot',
      frame:
        'Low angle of a trainer spotting or client lifting to highlight strength and focus.',
      light: 'Bright and even; avoid harsh shadows that obscure faces.',
      tip: 'Use sparingly but effectively to show power and community.',
    },
  ],
}
...
registerBrandPdfFonts()
const buf = await renderToBuffer(h(GymPhotosDoc))
