...
const outFile = join(outDir, 'cafe-3-photos.pdf')
...
const defaultCollagePath = join(repoRoot, 'public', 'pdf-assets', 'cafe-photos-collage-sample.png')
/** Native pixels of `cafe-photos-collage-sample.png` — update if you replace the asset. */
const COLLAGE_PX_W = 483
...
const S = StyleSheet.create({
  ...
  /** Match other photo one-sheets: one-line compact headline. */
  titleHeadline: {
    fontSize: 14.25,
 ...
 */
const content = {
  title: '3 Photos You Must Upload This Month: Café',
  subtitle: 'A visual blueprint for Google Maps & Yelp Presence',
  collageImageSrc: defaultCollagePath,
  angles: [
    {
      title: 'Exterior Storefront w/ Sign',
      frame: 'Landscape; include door + signage so people recognize you from the street.',
      light: 'Morning/late-afternoon; avoid blown-out sky.',
      tip: 'Tidy the entrance; remove clutter near the door.',
    },
    {
      title: 'Bright Wide Interior',
      frame: 'From a front corner toward the bar; keep lines straight.',
      light: 'Turn on house lights; let windows glow, not blast.',
    },
    {
      title: 'Hero Product Close-Up',
      frame:
        'Tight on your signature drink or plate; clean surface and minimal background clutter.',
      light: 'Soft natural or warm house light; avoid harsh flash on glass and steam.',
    },
  ],
}
...
registerBrandPdfFonts()
const buf = await renderToBuffer(h(CafePhotosDoc))
