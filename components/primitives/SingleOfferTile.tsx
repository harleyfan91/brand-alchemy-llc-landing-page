...
const SingleOfferTile: React.FC<SingleOfferTileProps> = ({
  offer,
  primaryHref,
  primaryLabel,
  ...
  const href = normalizeCheckoutHref(primaryHref);
  const label = offer.ctaLabel || primaryLabel;
  return (
    <article className="rounded-2xl border border-gray-200 p-5 sm:p-6">
      ...
      {href ? (
        ...
        <button
          type="button"
          disabled
          className="mt-5 inline-flex w-full cursor-not-allowed items-center justify-center rounded-full border border-gray-200 bg-gray-100 px-6 py-3 text-xs font-bold uppercase tracking-widest text-gray-500"
        >
