# Target audience (canonical reference)

**Purpose:** Single source of truth for who Brand Alchemy serves. Use this for site copy, product decisions, and any messaging so we do not have to infer the audience from memory.
...
**Cursor rules (do not forget)**

- Messaging and UI styling for this project are guided by **`.cursor/rules/brand-alchemy-messaging.mdc`** (`alwaysApply: true`), which points the AI at this file, the playbook, and **[BRAND_GUIDELINES.md](BRAND_GUIDELINES.md)** whenever you work on user-facing copy or visual patterns.
- If you add another canonical doc (e.g. SEO or proof guidelines), **update that rule** so new contributors and agents always load the same sources.
...
Be explicit so we do not drift toward the wrong tone or promises:
