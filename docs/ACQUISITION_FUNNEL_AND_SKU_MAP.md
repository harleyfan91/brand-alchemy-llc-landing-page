...
- [x] Audit every current CTA and decide which are **browse CTAs** vs **conversion CTAs** so the page has one primary path.
  Current browse / orientation CTAs: header nav links, hero **See the system**, **Articles** → `#articles`, product cards (e.g. **View Identity Kit**, **View guides & kits** → `/guides-and-kits`), and general contact.
